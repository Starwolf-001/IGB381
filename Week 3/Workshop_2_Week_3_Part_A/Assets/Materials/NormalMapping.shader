    ...
    SubShader 
	{
  		...
  		{ 
			...
			VSOutput VS_NormalMapping(VSInput a_Input) 
			{
				...
				// Scaling
				a_Input.pos.xyz += a_Input.nor * cos(_Theta);

				// calculate homogenous position
				output.pos = mul(rot, a_Input.pos);
			...
			float4 PS_NormalMapping(VSOutput a_Input) : COLOR
			{
				...
				// Dynamic Colours
				float4 adj = float4(sin(_Theta), cos(_Theta), tan(_Theta), 1.0f);
				colour = colour * adj;
